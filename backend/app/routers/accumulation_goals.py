...
async def list_goals(
    ...
    result = await db.execute(
        select(AccumulationGoal)
        .where(AccumulationGoal.user_id == user.id)
        .order_by(AccumulationGoal.id)
    )
...
async def create_goal(
    ...
    obj = AccumulationGoal(**data.model_dump(), user_id=user.id)
    db.add(obj)
    await db.commit()
